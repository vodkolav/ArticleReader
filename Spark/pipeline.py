...
from processing import preprocess_text_udf, split_text_into_chunks_udf, predict_batch_udf , sentences_schema, concat_waveforms
...
def batch_tts(microBatchDF):
    ...
    microBatchDF.select("window", "request_id", "text")


    # Apply UDF (returns an array of structs)
    df_chunks = microBatchDF.withColumn("chunks", split_text_into_chunks_udf(col("text")))
    ...
    if conf.test_run:
        chunks = df_exploded.filter(col("index").between(lit(301),lit(315)))
    ...
    chunks = chunks.groupBy("window", "request_id").applyInPandas(cum_text_volume, sentences_schema)
    ...
    assembled = processed.groupBy("window","request_id").agg(
    ...
    result = microBatchDF.join(assembled.select("window","request_id", "speech"), on=["window","request_id"], how="left")
    return result
